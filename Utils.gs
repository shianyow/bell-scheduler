...
function checkOfflineAndNotify() {
  ...
  const email = getConfigValue('NotifyEmail', '');
  
  if (!email) return; // 未設定 email 則不執行
  ...
  if (diffMinutes > offlineThreshold && !lastOfflineNotification) {
    // 離線通知：超過閾值且沒有發過離線通知
    sendNotificationEmail('離線', `前端已離線 ${Math.round(diffMinutes)} 分鐘`);
    updateStatusEvent('LastOfflineNotification');
  } else if (diffMinutes < recoveryThreshold && lastOfflineNotification) {
    // 恢復通知：小於閾值且有發過離線通知
    sendNotificationEmail('恢復', '前端設備已恢復連線');
